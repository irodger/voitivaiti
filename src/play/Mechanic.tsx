...
export function StepRenderer({step,readOnly=false,viewProgress}:{step:Step;readOnly?:boolean;viewProgress?:StepProgress}){const world=useWorld(),{t}=useI18n(),task=activeTask(world)!,p=viewProgress??task.progress[step.id],done=p.status==='completed',disabled=readOnly||done;
 ...
 {items&&<><p className="mechanic-instruction">{t(allocation?'ui.allocationHelp':ordered?'plan.reorderHelp':consequential?'fix.observations':'ui.selectHelp',{budget:step.budget??0})}</p><div className={`evidence-grid ${['dependency-map','architecture-diagram'].includes(step.type)?'dependency-nodes':''} ${step.type==='visual-compare'?'visual-options':''}`}>{items.map(item=><div className={`evidence-row ${!ordered&&p.draft.includes(item.id)?'selected':''}`} key={item.id}>{allocation?<><label htmlFor={`allocation-${item.id}`}>{t(item.labelKey)}</label><input id={`allocation-${item.id}`} type="number" min="0" max={step.budget} value={p.allocation[item.id]??0} disabled={disabled} onChange={e=>world.dispatch({type:'allocate',id:item.id,value:Number(e.target.value)})} aria-label={t('ui.amount',{item:t(item.labelKey)})}/></>:<button disabled={disabled} onClick={()=>world.dispatch({type:'draft',id:item.id})} aria-label={ordered&&!disabled?t('plan.moveFirst',{item:t(item.labelKey)}):undefined} aria-pressed={ordered?undefined:p.draft.includes(item.id)}>{ordered?<span className="order-number">{displayOrder.includes(item.id)?displayOrder.indexOf(item.id)+1:<GripVertical size={16}/>}</span>:<span className="evidence-check">{p.draft.includes(item.id)&&<Check size={15}/>}</span>}<span>{t(item.labelKey)}</span>{['dependency-map','architecture-diagram'].includes(step.type)&&<ArrowRight size={18}/>}</button>}</div>)}</div>{allocation&&<p className="allocation-count">{t('ui.allocated',{used:Object.values(p.allocation).reduce((a,b)=>a+b,0),budget:step.budget??0})}</p>}{!disabled&&<Button onClick={()=>world.dispatch({type:'submit'})}>{t(ordered?'plan.confirm':consequential?'fix.accept':'ui.submitPlan')}</Button>}</>}
