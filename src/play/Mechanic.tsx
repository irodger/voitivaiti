...
export function StepRenderer({step,readOnly=false,viewProgress}:{step:Step;readOnly?:boolean;viewProgress?:StepProgress}){const world=useWorld(),{t}=useI18n(),task=activeTask(world)!,p=viewProgress??task.progress[step.id],done=p.status==='completed',disabled=readOnly||done;
 ...
 {!step.actionFlow&&p.responseKey&&<div className={`response ${consequential?'decision-response':done?'success':'retry'}`} role="status">{!consequential&&<Check size={17}/>}<div>{t(consequential&&!done?'decision.pending':p.responseKey)}{!done&&!consequential&&<small>{t('ui.retry',{minutes:step.wrongMinutes})}</small>}</div></div>}
