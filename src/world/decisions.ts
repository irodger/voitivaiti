...
export function recordDecision(c:Campaign,task:Task,step:Step,immediate:Effect[]){
 ...
 const missed=step.id==='investigate'&&Array.isArray(step.solution)?step.solution.filter(id=>!p.draft.includes(id)):[];
 const explanationKey=task.templateId==='design-checkout'&&missed.includes('item-1')?'fix.missedTotal':missed.length?'fix.missed':undefined;
 ...
 if(missed.length)delayedEffects.push({target:'stress',value:2},{target:'techDebt',value:2});
 entries.push({explanationKey,decisionId,choiceId,choiceLabelKey:option?.labelKey??step.titleKey,selection,profession:ch.profession,characterId:ch.id,companyId:c.company!.id,projectId:task.projectId,day:c.life!.calendarDay,minute:c.time,npcId:step.speaker??(task.templateId.startsWith('design-')?'anya':tags.includes('ignoredQa')?'max':tags.includes('ignoredReview')?'ilya':'sergey'),tags,immediateEffects:structuredClone(immediate),delayedEffects,dueDay:c.life!.calendarDay+2+seed%3,resolved:false,context});
...
export function decisionOutcome(d:ProfessionalDecision){if(d.explanationKey)return d.explanationKey;return d.delayedEffects.some(e=>e.target==='stability'&&e.value<0)?'decision.followup.risk':d.delayedEffects.some(e=>e.target==='reputation')?'decision.followup.speed':'decision.followup.care';}
